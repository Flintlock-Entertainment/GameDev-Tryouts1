...
public class Oscillator : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (leftRight)
        {
            ...
            yTime = transform.position.y;
            transform.position = new Vector3(Mathf.Sin(xTime * objectSpeed) * motionLimit , yTime, zTime); // osilate the object left and right per frame * the speed and the motion limit.
        }
        else
        {
            xTime = transform.position.x;
